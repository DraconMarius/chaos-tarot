...
const resolvers = {
    ...
    Mutation: {
        ...
        createLog: async (
            ...
            //
            let prompt = `You are an agent simulated as an helpful API wizard: This is a ${readtype} reading and ${pulledCards} was pulled. Provide a sentence describing only the occult imagery (for text to image generation) of the card while avoiding any references of nudity or nakedness, then provide an unique explanation for this reading based on the card and its direction, and also 1 advice per Card you would share to clarify this specific reading. only provide a RFC8259 compliant JSON response following this format without deviation. {"upright":"true"/"false","card":"name","imagery":"imagery","meaning":"meaning","advice":"advice"};`

            //if there are user input
            if (readtype !== "daily") {
                prompt = `You are an agent simulated as an helpful API wizard only able to respond in RFC8259 compliant JSON format: This is a ${readtype} reading and ${pulledCards} was pulled asking for an answer to \"${question}\".  Provide a sentence describing only the occult imagery (for text to image generation) of the card while avoiding any references of nudity or nakedness, then provide an unique explanation for this specific card and the direction pulled and also provide a concise answer to the question. only provide a RFC8259 compliant JSON response following this format without deviation. {"upright":"true"/"false","card":"name","imagery":"imagery","meaning":"explanation","advice":"advice"};`
            }
